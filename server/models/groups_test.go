...
func testGroupToManyItems(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := GroupSlice{&a}
	if err = a.L.LoadItems(ctx, tx, false, (*[]*Group)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testGroupToManyMembers(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"member_groups\" (\"group_id\", \"member_id\") values (?, ?)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"member_groups\" (\"group_id\", \"member_id\") values (?, ?)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
func testGroupToManyAddOpItems(t *testing.T) {
	var err error
...
func testGroupToManySetOpMembers(t *testing.T) {
	var err error
...
func testGroupToManyRemoveOpMembers(t *testing.T) {
	var err error
...
var (
	groupDBTypes = map[string]string{`ID`: `INTEGER`, `Name`: `TEXT`}
	_            = bytes.MinRead
)
...
func testGroupsSliceUpdateAll(t *testing.T) {
	...
	if strmangle.StringSliceMatch(groupAllColumns, groupPrimaryKeyColumns) {
		...
		fields = strmangle.SetComplement(fields, groupGeneratedColumns)
	}
